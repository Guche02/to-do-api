...
@router.post("/add_todo/{user_token}")
async def create_todo_route(user_token:str,todo:TodoCreate,db:Session= Depends(get_db)):
    print(todo)
    todo=await create_todo_controller(user_token,todo,db)
    return todo

@router.patch("/update_todo/{task_id}/{user_token}")
async def update_todo_route(user_token:str,task_id:str,todo:TodoUpdate,db:Session= Depends(get_db)):
    print(todo)
...
@router.delete("/delete_todo/{todo_id}/{user_token}")
async def delete_todo(user_token:str,todo_id:str, db: Session = Depends(get_db)):
